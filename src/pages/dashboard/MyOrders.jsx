    ...
    <div className="space-y-10">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-2">
          <h1 className="text-4xl font-medium text-primary tracking-tighter">Purchase History</h1>
        ...
        <div className="hidden md:flex items-center gap-2 px-4 py-2 bg-emerald-50 border border-emerald-100 rounded-2xl">
          ...
          <span className="text-[10px] font-medium uppercase tracking-widest text-emerald-600">Secure Transactions</span>
        </div>
      </div>
              ...
              <div key={order._id} className="group p-8 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-8 hover:bg-gray-50 transition-all duration-300">
                <div className="flex items-center gap-6">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center gap-3">
                      <p className="text-2xl font-medium text-primary tracking-tight">{order.car?.name || order.car?.title || 'Luxury Vehicle'}</p>
                      <div className={`px-3 py-1 rounded-full text-[8px] font-medium uppercase tracking-widest ${order.paymentStatus === 'success' ? 'bg-emerald-500 text-white' : 'bg-amber-500 text-white'
                        }`}>
                        {order.paymentStatus}
                      </div>
                    </div>
                    <div className="flex items-center gap-6">
                      <div className="flex items-center gap-2">
                        <p className="text-[10px] font-medium text-gray-medium uppercase tracking-widest leading-none">Order ID</p>
                        <p className="text-xs font-bold text-primary">#{order._id.slice(-10).toUpperCase()}</p>
                      </div>
                      <div className="h-4 w-[1px] bg-gray-200" />
                      <div className="flex items-center gap-2">
                        <p className="text-[10px] font-medium text-gray-medium uppercase tracking-widest leading-none">Date</p>
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-8 w-full lg:w-auto">
                  <div className="text-left lg:text-right flex-1 lg:flex-none">
                    <p className="text-3xl font-medium text-primary">₦{order.price?.toLocaleString() || order.totalPrice?.toLocaleString()}</p>
                    <p className="text-[10px] font-medium text-gray-medium uppercase tracking-widest mt-1">Acquisition Price</p>
                  </div>
                  ...
                  <div className="flex items-center gap-3 w-full sm:w-auto">
                    {order.paymentStatus === 'pending' && order.status !== 'cancelled' ? (
                      <button
                        ...
                        className="flex-1 sm:flex-none h-14 px-8 rounded-2xl bg-accent text-primary text-[10px] font-medium uppercase tracking-widest flex items-center justify-center gap-2 hover:bg-primary/90 hover:text-white transition-all shadow-lg shadow-accent/20"
                      >
                        {isInitializing ? <Loader2 size={16} className="animate-spin" /> : <CreditCard size={16} />}
                        Complete Payment
                      </button>
                    ) : (
                      <button className="flex-1 sm:flex-none h-14 px-8 rounded-2xl bg-white border border-gray-200 text-[10px] font-medium uppercase tracking-widest flex items-center justify-center gap-2 hover:bg-primary hover:text-white hover:border-primary transition-all">
                        <Download size={16} />
                        Receipt
                      </button>
                    )}
