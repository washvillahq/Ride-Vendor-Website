...
const StatCard = ({ title, value, icon: Icon, color, bg, trend }) => (
  <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all group overflow-hidden relative">
    ...
    <div className="flex justify-between items-start relative z-10 mb-8">
      ...
      {trend && (
        <span className="flex items-center gap-1 text-[10px] font-black text-emerald-500 bg-emerald-50 px-3 py-1 rounded-full uppercase tracking-widest">
          +{trend}% <ArrowUpRight size={10} />
        </span>
      )}
    </div>
    <div className="relative z-10">
      <p className="text-xs font-black text-slate-400 uppercase tracking-[0.2em] leading-none mb-3">{title}</p>
         ...
         <div className="xl:col-span-2 space-y-6">
            <div className="flex items-center justify-between">
               <div className="flex items-center gap-3">
                  ...
                  <h2 className="text-2xl font-black text-slate-900 tracking-tight">Recent Rental Bookings</h2>
               </div>
               <Link to="/admin/bookings" className="text-xs font-black uppercase tracking-widest text-emerald-500 hover:text-emerald-600 transition-colors flex items-center gap-1">
            ...
            <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
               <Table>
                  <TableHeader className="bg-slate-50/50">
                     <TableRow>
                        <TableHead className="py-5 font-black uppercase tracking-widest text-[10px]">Client / Vehicle</TableHead>
                        <TableHead className="py-5 font-black uppercase tracking-widest text-[10px]">Scheduled</TableHead>
                        <TableHead className="py-5 font-black uppercase tracking-widest text-[10px]">Status</TableHead>
                        <TableHead className="py-5 font-black uppercase tracking-widest text-[10px] text-right">Revenue</TableHead>
                     </TableRow>
                  </TableHeader>
                  <TableBody>
                     {bookingsLoading ? (
                        ...
                        <TableRow key={booking._id} className="group hover:bg-slate-50/50 transition-colors">
                           ...
                           <TableCell className="py-5">
                              <StatusBadge status={booking.status} className="text-[9px] uppercase font-black px-3" />
                           </TableCell>
         ...
         <div className="space-y-6">
            <div className="flex items-center justify-between">
               <div className="flex items-center gap-3">
                  ...
                  <h2 className="text-2xl font-black text-slate-900 tracking-tight">Recent Sales</h2>
               </div>
               <Link to="/admin/orders" className="text-xs font-black uppercase tracking-widest text-orange-500 hover:text-orange-600 transition-colors flex items-center gap-1">
                  View All <ChevronRight size={14} />
               </Link>
            </div>
            ...
            <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-8 space-y-6">
               {ordersLoading ? (
                  ...
                  <div key={order._id} className="flex items-center justify-between p-4 rounded-3xl border border-slate-50 hover:bg-slate-50 transition-all group">
                     <div className="flex items-center gap-4">
                        <div className="h-12 w-12 bg-slate-900 rounded-2xl flex items-center justify-center text-white font-black overflow-hidden">
                           {order.car?.images?.[0] ? (
                             <img src={order.car.images[0].url} className="h-full w-full object-cover opacity-80" alt="Car" />
                           ) : <ShoppingBag size={20} />}
                        </div>
                        <div>
                           <p className="text-sm font-black text-slate-900 leading-none">{order.car?.title || 'Vehicle Order'}</p>
               ...
               <div className="pt-4 border-t border-slate-50">
                   <div className="bg-slate-900 rounded-3xl p-6 text-white relative overflow-hidden group">
                      ...
                      <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-2">System Status</p>
                      <h4 className="text-lg font-bold mb-4">Operational Status: Stable</h4>
                      <button className="px-4 py-2 bg-white text-black text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-white/90 transition-all">Review Audit Logs</button>
                   </div>
               </div>
            </div>
         </div>
