...
const HireCheckoutModal = ({ isOpen, onClose, car, initialData }) => {
  ...
  return (
    ...
    >
            ...
            return (
              <React.Fragment key={step.id}>
                <div className="flex flex-col items-center gap-2">
                  ...
                  <span className={`text-[10px] font-bold uppercase tracking-wider ${isActive ? 'text-primary' : 'text-gray-medium'}`}>
                    {step.title}
                  </span>
                </div>
                {i < STEPS.length - 1 && (
                  <div className={`flex-1 h-0.5 rounded-full mb-4 ${isCompleted ? 'bg-emerald-500' : 'bg-gray-100'}`} />
                )}
              </React.Fragment>
            );
          })}
          ...
          <div className="lg:col-span-8 space-y-8">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {services.length > 0 ? (
                      services.map(service => (
                        ...
                        >
                          <div className="flex items-start justify-between gap-2">
                            <div className="space-y-1">
                              <h4 className="font-bold text-primary text-sm leading-tight">{service.name}</h4>
                              <p className="text-[10px] text-gray-medium uppercase tracking-wide">{service.description}</p>
                            </div>
                            <div className={`shrink-0 h-5 w-5 rounded-full border-2 flex items-center justify-center transition-colors ${selectedServices.includes(service._id) ? 'bg-accent border-accent text-primary' : 'border-gray-200'
                              }`}>
                      ...
                      <div className="col-span-full py-10 text-center space-y-2">
                        <p className="text-gray-medium font-bold uppercase text-xs">No extra services available.</p>
                      </div>
                    )}
                  </div>
                )}
            ...
            {currentStep === 2 && (
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                ...
                <div className="bg-gray-50 p-4 sm:p-6 rounded-3xl border border-gray-100 flex justify-center">
                  <DayPicker
                    mode="multiple"
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-gray-50 rounded-2xl p-4 border border-gray-100">
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="flex justify-between items-center text-sm font-bold text-primary pt-2 border-t border-gray-200 mt-2">
      ...
      {/* Footer Navigation */}
      <div className="bg-gray-50 border-t border-gray-100 p-4 sm:p-6 shrink-0 flex items-center justify-between">
        <Button
          variant="ghost"
          onClick={handleBack}
        ...
        {currentStep < 4 ? (
          <Button
            onClick={handleNext}
            className="px-8 bg-primary hover:bg-primary/90 text-white rounded-xl text-xs uppercase tracking-widest shadow-md"
          >
            Continue
          </Button>
        ) : (
